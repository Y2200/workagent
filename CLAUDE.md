...
- **P6-2 CI/CD 容器化改造已完成并全链验证（2026-09-12）**：CI 构建前后端镜像推 Docker Hub（tag=commit SHA）→ CD 只 `pull` + `up -d`；`Dockerfile.frontend` = Nginx + dist 融合镜像（唯一入口 80/443，TLS + `/api` 反代 `backend:8000`）；`deploy/nginx/conf.d/` 配置进镜像；certbot 容器化 + 宿主机 systemd timer 续期；后端与 6 个基础设施服务定义未动
- **生产服务器切换已完成**：宿主机 Nginx 停用、frontend 容器接管 80/443、证书迁至 `/opt/work-agent/certbot/{conf,www}` 并由 `cert-renew.timer` 续期；`https://wkcp.online` 返回 200、`api.wkcp.online/health` 正常；deploy job 已全绿
  - ⚠️ **长期前提**：`workagent-backend` 是 **private** 仓库 → 服务器须保持 Docker Hub 登录态（凭据丢失时部署会停在 `pull`，属安全失败：`up -d` 前中止，生产继续跑旧版本）
  - ⚠️ **不要在服务器上执行 `git pull/checkout/stash`**：服务器留有 P6-1 源码树，会恢复旧 compose 破坏新架构
  - 运维细节见 `deploy/README.md`「当前状态」；原「milvus_store 接线待批准」阻塞项**已解除**（`rag/milvus_store.py:16` 已用 `settings.milvus_uri`）
- **下一步**：回滚演练（`.deploy_history` ≥2 条后做）→ 服务器旧源码清理 → 可选加固（deploy job 内自动 `docker login`）；业务侧：前端接入治理看板、企业微信正式接入、Celery（见 PROJECT_CONTEXT.md）
